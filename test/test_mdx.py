import common
from seqlablib.mdx import Workup
import sqlite3
...
class MockMDX(object):
    ...
    def update_by_sequence_key(self, key, path):
        w = ws[self.seqkeys.index(key)]
        ws[self.seqkeys.index(key)] = Workup(accession=w.accession,
                                             workup=w.workup,
                                             pat_name=w.pat_name,
                                             amp_name=w.amp_name,
                                             path=path)
